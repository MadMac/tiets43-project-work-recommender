...
def create_game_from_xml(xml_root, game_id, html_parser):
    ...
    if game_type not in ["boardgame", "boardgameexpansion"]:
        log.debug("unsupported type {} for item id {}".format(
            game_type, game_id))
    ...
    for e in xml_root.findall("link[@type='boardgameexpansion']"):
        try:
            ...
            raise BGGApiError(
                "malformed XML element ('link type=boardgameexpansion')")
    ...
    if stats is not None:
        ...
        for poll in polls:
            if poll.attrib.get("name") == "suggested_numplayers":
                ...
                data["suggested_players"]['total_votes'] = poll.attrib.get(
                    'totalvotes')
...
def add_game_comments_from_xml(game, xml_root):
    ...
    if xml_root is not None:
        ...
        raise BGGApiError("Error loading comments")
    # print("Children:")
