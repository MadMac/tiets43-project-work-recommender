...
COLLECTION_SUBTYPES = ["boardgame", "boardgameexpansion",
...
class BGGCommon(object):
    ...
    def _get_game_id(self, name, game_type, choose):
        ...
        if choose not in [BGGChoose.FIRST, BGGChoose.RECENT, BGGChoose.BEST_RANK]:
            raise BGGValueError(
                "invalid value for parameter 'choose': {}".format(choose))
    ...
    def user(self, name, progress=None, buddies=True, guilds=True, hot=True, top=True, domain=BGGRestrictDomainTo.BOARD_GAME):
        ...
        data["lastlogin"] = xml_subelement_attr(root,
                                                "lastlogin",
                                                convert=lambda x: datetime.datetime.strptime(
                                                    x, "%Y-%m-%d"),
                                                quiet=True)
        ...
        try:
            call_progress_cb(progress, max(user.total_buddies,
                                           user.total_guilds), max_items_to_fetch)
        except:
            return user
        ...
        while max(user.total_buddies, user.total_guilds) < max_items_to_fetch:
            ...
            try:
                call_progress_cb(progress, max(
                    user.total_buddies, user.total_guilds), max_items_to_fetch)
            except:
                break
            ...
            if not added_buddy and not added_guild:
                log.debug(
                    "didn't add any buddy/guild after fetching page {}, stopping here".format(page))
                break
    ...
    def plays(self, name=None, game_id=None, progress=None, min_date=None, max_date=None, subtype=BGGRestrictPlaysTo.BOARD_GAME):
        ...
        if name and game_id:
            raise BGGValueError(
                "can't retrieve by user and by game at the same time")
    ...
    def collection(self, user_name, subtype=BGGRestrictCollectionTo.BOARD_GAME, exclude_subtype=None, ids=None, versions=False,
        ...
        if exclude_subtype is not None:
            ...
            if subtype == exclude_subtype:
                raise BGGValueError(
                    "incompatible 'subtype' and 'exclude_subtype'")
...
class BGGClient(BGGCommon):
    ...
    def game(self, name=None, game_id=None, choose=BGGChoose.FIRST, versions=False, videos=False, historical=False,
        ...
        log.debug("retrieving game id {}{}".format(
            game_id, " ({})".format(name) if name is not None else ""))
        ...
        if xml_root is None:
            msg = "invalid data for game id: {}{}".format(
                game_id, "" if name is None else " ({})".format(name))
            raise BGGApiError(msg)
        ...
        if not comments and not rating_comments:
            return game
